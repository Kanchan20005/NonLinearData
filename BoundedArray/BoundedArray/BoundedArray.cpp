...
BoundedArray::BoundedArray(int numElements, int maxNumElements){
	...
	for (int i = 0; i < maxNumElements; i++)
		elements[i] = 0;
	double* elements_dynamic = new double[maxNumElements];  //New operator

	for (int i = 0; i < numElements && i < maxNumElements; i++) {
		elements_dynamic[i] = elements[i];
	}
	delete elements;  //Delete operator.
